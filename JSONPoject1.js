...
If you don't see data below  the label "Resource  for ditto", you will need  to click  the blue 

Find the following

1. All the abilities
   > pokeTest.abilities OR
   > for(var i=0; i<poke.abilities.length; i++)
      console.log(poke.abilities[i])
2. The base experience 
  > pokeTest.base_experience
3. The height
  > pokeTest.height
4. The url for the item "metal-powder"
  > pokeTest.held_items[0].item.url
5. All the sprites
  > pokeTest.sprites
6. The different stat names 
  >pokeTest.stats[0].stat.name
  >pokeTest.stats[1].stat.name

]>for(var i=0; i<poke.stats.length; i++)
  console.log(poke.stats[i].stat.name)

7. The slot for the types
   > pokeTest.types[0].slot
8. The weight
  >pokeTest.weight
9. The first version _group_details
  > pokeTest.moves[0].version_group_details
